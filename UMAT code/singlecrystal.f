...
C     VARIBLES USED WITHIN THIS SUBROUTINE
      ...
      REAL(8) :: DSIGDPSI(6,6), DUM3X3_(3,3)
      ...
      REAL(8) :: LE(3,3), WE(3,3), WP(3,3), DR(3,3)
...
C         SCHMID DYADIC
          DO I=1,6
              DO J=1,6
                  SCHMIDXSCHMID(IS,I,J)=SNI(I)*SNI(J)
              ENDDO
          ENDDO
C
      ENDDO      
...
C     CONVERT TO 3X3 MATRIX
      CALL VECMAT6(SIGMATRVEC,SIGMATR)
C
C     STRESS GUESS (FULLY-PLASTIC)
      SIGMAVEC = SIGMAVEC_T
C
C     CONVERT STRESS TO A MATRIX
      CALL VECMAT6(SIGMAVEC,SIGMA)   
C      
...
C     VECTORIZE TRIAL STRESS
      CALL MATVEC6(SIGMATR,SIGMATRVEC)
C
C
...
C                 REF. https://doi.org/10.1016/0022-5096(92)80003-9
                  DO I=1,6
                      IF (ABS(DSIGMAVEC(I)).GT.2.*CRIT*TAUC0) THEN
                          DSIGMAVEC(I)=
     +2.*CRIT*TAUC0*SIGN(1.,DSIGMAVEC(I))
                      END IF
                  END DO
C
C                 UPDATE STRESS
...
C
C         ROTATION UPDATE
C         ACCORDING TO HUGHES AND WINGET
C         Q = (I - 1/2 DW)^-1 * (I + 1/2 DW)
          DR = I3X3 - 0.5 * WE * DT
          CALL INVERSE3X3(DR,DUM3X3,DUM)
          DUM3X3_ = I3X3 + 0.5 * WE * DT
          DR=0.
          DO I=1,3
              DO J=1,3
                  DO K=1,3
                      DR(I,J) = DR(I,J) +
     + DUM3X3(I,K) * DUM3X3_(K,J)
                  END DO
              END DO
          END DO
C
...
C         CONVERT STRESS TO A MATRIX
          CALL VECMAT6(SIGMAVEC,SIGMA)   
C
C         CO-ROTATIONAL STRESS TO CAUCHY STRESS
C         SIGMA = SIGMA + W * SIGMA - SIGMA * W
          DUM3X3=SIGMA
          DO I=1,3
              DO J=1,3
     ...
     + WE(I,K) * DUM3X3(K,J) * DT
          ...
          DO I=1,3
              DO J=1,3
                  DO K=1,3
                      SIGMA(I,J)=SIGMA(I,J)-
     + DUM3X3(I,K) * WE(K,J) * DT
                  END DO
              END DO
          END DO
C
C         VECTORIZE TRIAL STRESS
          CALL MATVEC6(SIGMA,SIGMAVEC)
C
C
